...
server's surface area defines the agent's action space, and the
...
- Both tools operate within the session's scope. They may not
  ...
  (for writes).
- Tool errors must be reported as MCP tool errors, not as server
  crashes. The server continues serving after a tool error.
